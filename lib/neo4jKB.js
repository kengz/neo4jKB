 ...
 * @private
 * @param  {JSON} options An optional JSON with the fields {NEO4J_AUTH: 'neo4j:neo4j', NEO4J_ADDR: 'localhost:7474'} containing username:password, the host address, and port number for neo4j
 ...
 * @param  {Array} propDistLabel Array containing pair of (JSON)prop-(string)label or (string)dist-(string)label.
 * @param  {string} [name='a'] Name of the identifier.
 * @return {string}      The result literal map string.
 *
 * @example
 ...
 *
 */
function literalize(propDistLabel, name) {
  name = name || 'a'
  var part = sortPropLabel(propDistLabel),
    propDist = _.get(part, '0.0'),
    prop = _.isPlainObject(propDist) ? propDist : undefined,
    prop = literalizeProp(prop, 'prop_'+name),
    dist = KB.cons.stringifyDist(propDist),
    Label = _.get(part, '1.0'),
    LabelStr = KB.cons.stringifyLabel(Label, name);
  return `${name}${LabelStr}${prop}${dist}`
}
