 ...
 */
function pushNode(propLabel) {
  var part = sortPropLabel(propLabel),
    prop = _.get(part, '0.0'),
    Label = _.get(part, '1.0'),
    LabelStr = cons.stringifyLabel(Label);

  if (cons.pass(prop)) {
    return [
      // check existence, find by hash
      'MERGE (a' + LabelStr + ' {hash: {prop}.hash}) ' +
      // create if no hash matched
      'ON CREATE SET a = {prop}, a.created_by={prop}.updated_by, a.created_when={prop}.updated_when ' +
      // update if hash matched
      'ON MATCH SET a += {prop} RETURN a', {
        prop: prop
      }
    ]
  } else {
 ...
 * 
 */
// var addNode = _.partial(batchQuery, pushNode)
var addNode = _.partial(batchQuery, pushNode)
...
// // addNode(propA, 'alpha').then(_.flow(JSON.stringify, console.log));
// // addNode([propA, 'alpha']).then(_.flow(JSON.stringify, console.log))
// // XXXXXaddNode([propA, 'alpha'], [propB, 'alpha']).then(_.flow(JSON.stringify, console.log))
// addNode([[[propA, 'alpha']], [[propB, 'alpha']]]).then(_.flow(JSON.stringify, console.log))
 ...
 */
function pullNode(propLabel) {
  var part = sortPropLabel(propLabel),
    prop = _.get(part, '0.0'),
    Label = _.get(part, '1.0'),
    LabelStr = cons.stringifyLabel(Label);

  return ['MATCH (a' + LabelStr + literalizeProp(prop) + ') RETURN a', _.pickBy({
    prop: prop
  })]
}
 ...
 * 
 */
var getNode = _.partial(batchQuery, pullNode)
 ...
 */
function pushEdge(propLabelA, propLabelE, propLabelB) {
 ...
 * 
 */
var addEdge = _.partial(batchQuery, pushEdge)
 ...
 */
function pullEdge(propLabelA, propLabelE, propLabelB) {
...
function pull(propLabelA, propDistLabelE, propLabelB, wOp, sOp, rOp, pOp) {
  ...
  if (!rOpStr) {throw new Error("You must include a RETURN|DELETE|DETACH DELETE statement.")};
