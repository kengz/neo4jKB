...
var cons = {
   ...
   *
   */
  legalize: function(prop, msg, hash_by) {
    if (_.isString(prop)) {
      prop = {
        name: prop,
        hash_by: 'name'
    ...
    // fix if msg is string, but still must be precise
    if (_.isString(msg)) {
      try {
        msg = _.set({}, 'envelope.user.id', user.getId(msg))
      } catch (e) {
        console.log(e)
        console.log('the "msg" for cons.legalize must be a hubot msg object, or you must provide a global user.getId(msg) method.')
      }
    };
    var hash_key = _.find([msg, hash_by], _.isString)
    if (hash_key) {
